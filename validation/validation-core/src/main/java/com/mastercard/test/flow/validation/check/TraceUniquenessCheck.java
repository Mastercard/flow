 ...
 */
public class TraceUniquenessCheck implements Validation {
	...
	public String explanation() {
		return "All flows in a model have a unique trace";
	}
		...
		for( int i = 0; i < flows.length; i++ ) {
			...
			for( int j = i + 1; j < flows.length; j++ ) {
				...
				checks.add( new Check( this,
						...
						() -> {
							if( a.meta().trace().equals( b.meta().trace() ) ) {
								return new Violation( this, "Shared trace", null, null )
										.offender( a )
										.offender( b );
							}
							return null;
